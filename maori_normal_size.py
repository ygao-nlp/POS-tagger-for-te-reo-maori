...
from Maori.maori_data import extend_TEM, MaoriDataset
...
"""# Create dataset"""
train_len = int(0.8 * len(ex_output))
dev_len = int(0.1 * len(ex_output))
test_len = int(0.1 * len(ex_output))

# data for crf and rf
shallow_train = ex_output[: train_len]
shallow_dev = ex_output[train_len:train_len + dev_len]
shallow_test = ex_output[train_len + dev_len:]

# data for bilstm and bilstm+crf
deep_train_word, deep_train_tag = ex_words[:train_len], ex_tags[:train_len]
deep_dev_word, deep_dev_tag = ex_words[train_len:train_len + dev_len], ex_tags[train_len:train_len + dev_len]
deep_test_word, deep_test_tag = ex_words[train_len + dev_len:], ex_tags[train_len + dev_len:]
...
for epoch in range(N_EPOCHS):
  ...
  if valid_loss < best_valid_loss:
      ...
      torch.save(bilstm.state_dict(), 'ex_bilstm.pt')
  if (epoch+1) % 10 == 0:
    print(f'Epoch: {epoch+1:02} | Epoch Time: {epoch_mins}m {epoch_secs}s')
    print(f'\tTrain Loss: {train_loss:.3f} | Train Acc: {train_acc*100:.2f}%')
    print(f'\t Val. Loss: {valid_loss:.3f} |  Val. Acc: {valid_acc*100:.2f}%')

bilstm.load_state_dict(torch.load('ex_bilstm.pt'))
...
for epoch in range(N_EPOCHS):
  ...
  if valid_loss < best_valid_loss:
    best_valid_loss = valid_loss
    torch.save(bilstm_crf.state_dict(), 'ex_bilstm_crf.pt')
  if (epoch+1) % 10 == 0:
    end_time = time.time()
    epoch_mins, epoch_secs = epoch_time(start_time, end_time)
    print(f'Epoch: {epoch+1:02} | Epoch Time: {epoch_mins}m {epoch_secs}s')
...
bilstm_crf.load_state_dict(torch.load('ex_bilstm_crf.pt'))
